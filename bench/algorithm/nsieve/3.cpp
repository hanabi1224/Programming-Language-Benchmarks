...
// An even faster C++ nsieve using calculation as in 3.cl, by Bela Pecsek
...
void nsieve(std::size_t max) {
  ...
  std::size_t num_entries = (max >> 1) + (max % 2) - 1;
  is_prime.assign(num_entries, 1u);
  // cutoff for sieve, min. divisor <= sqrt(max)
  std::size_t cutoff = std::min((std::size_t(sqrt(max)) / 2), num_entries);
  std::size_t i = 0;
  for (; i < cutoff; ++i) {
    if (is_prime[i] == 0u) continue;

    std::size_t offset = 2 * i * (3 + i) + 3;
