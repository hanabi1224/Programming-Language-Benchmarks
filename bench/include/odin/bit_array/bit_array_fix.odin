...
get :: proc(ba: ^Bit_Array, #any_int index: uint) -> (res: bool, ok: bool) {
	...
	if ba == nil || idx < 0 { return false, false }
...
/*
	...
	`set` does not automatically resizes the Bit Array to accommodate the requested index if needed.
*/
set :: proc(ba: ^Bit_Array, #any_int index: uint) -> (ok: bool) {
	idx := int(index) - ba.bias

	if ba == nil || idx < 0 { return false }
...
*/
create :: proc(max_index: int, min_index := 0) -> (res: ^Bit_Array, ok: bool) #optional_ok {
...
*/
@(private="file")
resize_if_needed :: proc(ba: ^Bit_Array, legs: int) -> (ok: bool) {
