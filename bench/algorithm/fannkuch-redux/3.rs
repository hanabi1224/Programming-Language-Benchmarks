...
// non-parallel version of 3-m.rs on https://programming-language-benchmarks.vercel.app/
// which in turn inspired by C++ #6 SIMD implementation by Andrei Simion
...
fn fannkuchredux(n: usize) -> (i32, i32) {
    ...
    (0..n).map(|rotate_count| {
        ...
        (0..n - 1).map(|rotate_count| {
            ...
            (checksum, max_rev)
        }).fold((0, 0), |(cs1, mr1), (cs2, mr2)| (cs1 + cs2, max(mr1, mr2)))
    }).fold((0, 0), |(cs1, mr1), (cs2, mr2)| (cs1 + cs2, max(mr1, mr2)))
}
