...
fn perm_with_count(n: i8, count: &[i8; 16]) -> V {
...
fn next_permutation(perm: &V, count: &mut [i8], size: usize) -> Option<V> {
    let mut r = 0;
    let mut none = true;
    for i in 0..size {
        if count[i] != 1 {
    ...
    let next_perm = shuffle(perm, &NEXT_PERM_MASKS[r + 1]);
    count[r] -= 1;
    for i in 0..r {
        count[i] = (i + 1) as i8;
    }
