    ...
    (flet ((position-deltas ()
	   ...
	   (force-magnitudes ()
	     ...
	     (loop for i of-type fixnum below +ROUNDED_INTERACTIONS_COUNT+ by 2
		   do (let* ((pdx (f64.2-aref position_Deltas-x i))
			     (pdy (f64.2-aref position_Deltas-y i))
			     (pdz (f64.2-aref position_Deltas-z i))
			     (distance_Squared (f64.2+ (f64.2* pdx pdx) (f64.2* pdy pdy)
						       (f64.2* pdz pdz)))
			     (magnitude (f64.2/ (f64.2* (f64.2-sqrt distance_Squared)
						        distance_Squared))))
			(setf (f64.2-aref magnitudes i) magnitude))))
	   (velocities ()
